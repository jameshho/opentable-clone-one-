...
export default async function handler(
  ...
  await prisma.$executeRaw`TRUNCATE "Review" RESTART IDENTITY CASCADE;`
  await prisma.$executeRaw`TRUNCATE "Item" RESTART IDENTITY CASCADE;`
  await prisma.$executeRaw`TRUNCATE "Restaurant" RESTART IDENTITY CASCADE;`
  await prisma.$executeRaw`TRUNCATE "Location" RESTART IDENTITY CASCADE;`
  await prisma.$executeRaw`TRUNCATE "Cuisine" RESTART IDENTITY CASCADE;`
  await prisma.$executeRaw`TRUNCATE "User" RESTART IDENTITY CASCADE;`
  await prisma.$executeRaw`TRUNCATE "Booking" RESTART IDENTITY CASCADE;`
  await prisma.$executeRaw`TRUNCATE "BookingsOnTables" RESTART IDENTITY CASCADE;`
  ...
  // 'const vivaanId = restaurantIds['Vivaan - fine Indian']'
  // 'const RamaKrishnaId = restaurantIds['RamaKrishna Indian']'
  // 'const coconutLagoonId = restaurantIds['Coconut Lagoon']'
  // 'const lastTrainToDelhiId = restaurantIds['Last Train to Delhi']'
  // 'const adrakYorkvilleId = restaurantIds['Adrak Yorkville']'
  // 'const curryishTavernId = restaurantIds['Curryish Tavern']'
  // 'const utsavId = restaurantIds['Utsav']'
  // 'const pukkaId = restaurantIds['Pukka']'
  // 'const kamasutraIndianId = restaurantIds['Kamasutra Indian']'
  // 'const eldoradoTacoId = restaurantIds['Eldorado Taco']'
  // 'const laBartolaId = restaurantIds['La Bartola']'
  // 'const elCatrinId = restaurantIds['El Catrin']'
  // 'const mariachisId = restaurantIds['3 Mariachis']'
  // 'const canoRestaurantId = restaurantIds['Cano Restaurant']'
  // 'const bluRistoranteId = restaurantIds['Blu Ristorante']'
  // 'const stelvioId = restaurantIds['Stelvio']'
  // 'const sofiaId = restaurantIds['Sofia']'

  const ress = ['Vivaan - fine Indian', 'RamaKrishna Indian', 'Coconut Lagoon', 'Last Train to Delhi', 'Adrak Yorkville', 'Curryish Tavern', 'Utsav', 'Pukka', 'Kamasutra Indian', 'Eldorado Taco', 'La Bartola', 'El Catrin', '3 Mariachis', 'Cano Restaurant', 'Blu Ristorante', 'Stelvio', 'Sofia']
  ...
  const restaurantIds: Record<string, number> = restaurantNames.reduce((acc, name) => {
    const id = restaurants.find((restaurant) => restaurant.name === name)?.id || 1;
    return {
      ...acc,
      [`${name}`]: id,
    };
    ...
    data: [
      {
        ...
        price: "$18.00",
        restaurant_id: restaurantIds['Vivaan - fine Indian'],
      },
      {
      ...
      },
      {
        ...
        price: "$16.99",
        restaurant_id: restaurantIds['RamaKrishna Indian'],
      },
      {
        name: "Chicken Tikka",
        description:
      ...
      },
      {
        ...
        price: "$25.00",
        restaurant_id: restaurantIds['Coconut Lagoon'],
      },
      {
      ...
      },
      {
        ...
        price: "$12.00",
        restaurant_id: restaurantIds['Last Train to Delhi'],
      },
      {
      ...
      },
      {
        ...
        price: "$18.00",
        restaurant_id: restaurantIds['Adrak Yorkville'],
      },
      {
        name: "papad ki tokri",
      ...
      },
      {
        ...
        price: "$32.00",
        restaurant_id: restaurantIds['Curryish Tavern'],
      },
      {
      ...
      },
      {
        ...
        price: "$4.00",
        restaurant_id: restaurantIds['Utsav'],
      },
      {
      ...
      },
      {
        ...
        price: "$23.00",
        restaurant_id: restaurantIds['Pukka'],
      },
      {
        name: "Short Ribs",
        description:
      ...
      },
      {
        ...
        price: "$8.99",
        restaurant_id: restaurantIds['Kamasutra Indian'],
      },
      {
      ...
      },
      {
        ...
        price: "$23.00",
        restaurant_id: restaurantIds['El Catrin'],
      },
      {
      ...
      },
      {
        ...
        price: "$23.00",
        restaurant_id: restaurantIds['3 Mariachis'],
      },
      {
      ...
      },
      {
        ...
        price: "$24.00",
        restaurant_id: restaurantIds['Cano Restaurant'],
      },
      {
      ...
      },
      {
        ...
        price: "$29.00",
        restaurant_id: restaurantIds['Blu Ristorante'],
      },
      {
      ...
      },
      {
        ...
        price: "$24.00",
        restaurant_id: restaurantIds['Stelvio'],
      },
      {
  ...
  function createUser(firstName: string, lastName: string) {
    const email = `${firstName.toLowerCase()}_${lastName.toLowerCase()}@${getRandomDomain()}.com`;
    const city = getRandomCity();
    //Password123
    const password = "$2b$10$EowLa/phg2Ekn6APJsv6DOPbrH8H6qSTY9Axc6KxiPoiYPQC3lZhy";
  ...
  function getRandomCity() {
    const cities = ["toronto", "ottawa", "niagara"];
  ...
  const userRachel =
    await prisma.user.create({
      data:
      {
        ...
        city: getRandomCity(),
        password: "$2b$10$EowLa/phg2Ekn6APJsv6DOPbrH8H6qSTY9Axc6KxiPoiYPQC3lZhy",
        phone: "1112223333",
      }
    });
  ...
  const userDaniel =
    await prisma.user.create({
      data:
        createUser("Daniel", "Phillips")
    });

  const userEmily =
    await prisma.user.create({
      data:
        createUser("Emily", "Nguyen")
  ...
  const userSophia =
    await prisma.user.create({
      data:
        createUser("Sophia", "Singh")

    });
  const userJason =
    await prisma.user.create({
      data:
        createUser("Jason", "Chan")

    });
  const userAva =
    await prisma.user.create({
      data:
        createUser("Ava", "Martinez")
  ...
  const userEthan =
    await prisma.user.create({
      data:
        createUser("Ethan", "Mitchell")

    });
  const userList = [userRachel, userDaniel, userEmily, userJoshua, userOlivia, userLiam, userSophia, userJason, userAva, userNoah, userEthan]


  function getRandomComment() {
    const comments = ["The food was amazing and the service was top-notch. I highly recommend this restaurant.",
      "I had high hopes for this restaurant, but unfortunately the food was disappointing.",
      "The ambiance of the restaurant was lovely and the food was delicious. I will definitely be coming back.",
      "The service was slow and the food was just okay. I wouldn't recommend this restaurant.",
      "The restaurant had a great atmosphere and the staff was friendly, but the portions were small for the price.",
      "The food was incredible and the presentation was beautiful. I will definitely be back.",
      "The restaurant had a cozy atmosphere and the food was flavorful, but the prices were a bit high.",
      "I was pleasantly surprised by the variety of options on the menu and the quality of the food. A great restaurant!",
      "The service was excellent and the food was outstanding. I highly recommend this restaurant.",
      "The restaurant had a great view and the food was delicious. However, the noise level was a bit high for my liking.",
      "This restaurant had a great ambiance and the staff were very accommodating. The food, however, was just okay.",
      "The food at this restaurant was incredible and the prices were reasonable. I highly recommend it.",
      "The atmosphere of this restaurant was lovely, but the service was slow and the food was mediocre.",
      "The portions at this restaurant were quite small for the price, but the flavors were unique and tasty.",
      "The staff at this restaurant were friendly and attentive, and the food was delicious. A great dining experience overall.",
      "This restaurant had a great selection of vegetarian options, and the food was flavorful and well-prepared.",
      "The service at this restaurant was top-notch and the food was absolutely amazing. Definitely worth a visit.",
      "The restaurant had a beautiful decor and a great view, but the food was disappointing and overpriced.",
      "The menu at this restaurant was quite limited, but the dishes that were offered were executed perfectly.",
      "The restaurant had a cozy atmosphere and the food was well-prepared, but the noise level was a bit high for a romantic dinner.",
      "Excellent food and service. Highly recommend!",
      "Nice place for a date night. The food was amazing.",
      "Restaurant was loud and crowded. Food is overpriced.",
      "Friendly server and a good wine selection.",
      "The food was great, but the price is a little high.",
      "Had a great time here. Our server was friendly and the food was great.",
      "The service was poor and we had to wait a long time for our food.",
      "Wonderful food and service.",
      "Great food and service. You can't ask for much more from a restaurant.",
      "Great service! Delicious food!",
      "The steak was spot on. Great dessert.",
      "Came here with a friend who had alsonever been here before. We loved it!",
      "Very nice evening spent with my family.", "The food was deliscious and service very good. Highly recommend.", "Enjoyed our drinks, dinner and dessert.", "The food was delicious, we had such a great experience and our server was top notch. ",
      "Our waiter was amazing! Great experience overall!", "The place is a little too cold but overall we really enjoyed the evening",
      "Service was terrible but the food was good.", "Fantastic food and excellent selection. Everything was fresh - and the scones were still warm!"]
  ...
  function getRandomRestaurant() {
    const ress = ['Vivaan - fine Indian', 'RamaKrishna Indian', 'Coconut Lagoon', 'Last Train to Delhi', 'Adrak Yorkville', 'Curryish Tavern', 'Utsav', 'Pukka', 'Kamasutra Indian', 'Eldorado Taco', 'La Bartola', 'El Catrin', '3 Mariachis', 'Cano Restaurant', 'Blu Ristorante', 'Stelvio', 'Sofia']
    const randomIndex = Math.floor(Math.random() * ress.length);
    return ress[randomIndex];
  }
    ...
    for (let i = 0; i < userList.length; i++) {
      for (let u = 0; u < 5; u++) {
        emptyArr.push({
          ...
          restaurant_id: restaurantIds[getRandomRestaurant()],
  ...
  //currently adding 3 tables per restaurant
  // const restaurantsSeats = [
  //   {
  //     name: 'Vivaan - fine Indian',
  //     tables: [4, 4, 2]
  //   },
  ...
  // ];
  // const tableData = [];
  // for (const restaurant of restaurantsSeats) {
  //   for (const table of restaurant.tables) {
  //     tableData.push({
  //       restaurant_id: restaurantIds[restaurant.name],
  //       seats: table
  //     });
  //   }
  // }
  ...
  //   data: [
  //     {
  //       restaurant_id: restaurantIds['Vivaan - fine Indian'], //50
  //       seats: 4,
  //     },
  //     {
  //       restaurant_id: restaurantIds['Vivaan - fine Indian'],
  ...
  //     },
  //     {
  //       restaurant_id: restaurantIds['RamaKrishna Indian'], //50
  //       seats: 4,
  //     },
  //     {
  //       restaurant_id: restaurantIds['RamaKrishna Indian'],
  ...
  const booking1 = await prisma.booking.create({
    ...
    {
      number_of_people: 4,
      booking_time: new Date('2023-04-13T17:00:00Z'),
      booker_email: "rachel_evans@yahoo.com",
      booker_phone: "1112223333",
      booker_first_name: "Rachel",
      booker_last_name: "Evans",
      booker_occasion: 'Birthday',
      booker_request: 'Birthday cake will be delivered ahead of time.',
      restaurant_id: restaurantIds['Vivaan - fine Indian'],
      user_id: 1
    }
  });
  const booking2 = await prisma.booking.create({
    ...
    {
      number_of_people: 4,
      booking_time: new Date('2023-04-15T16:00:00Z'),
      booker_email: "rachel_evans@yahoo.com",
      booker_phone: "1112223333",
      booker_first_name: "Rachel",
      booker_last_name: "Evans",
      booker_occasion: 'Date',
      booker_request: 'Table by the window',
      restaurant_id: restaurantIds['Stelvio'],
      user_id: 1
    }
  });
  const booking3 = await prisma.booking.create({
    ...
    {
      number_of_people: 4,
      booking_time: new Date('2023-04-21T18:00:00Z'),
      booker_email: "rachel_evans@yahoo.com",
      booker_phone: "1112223333",
      booker_first_name: "Rachel",
      booker_last_name: "Evans",
      booker_occasion: 'Business Dinner',
      booker_request: 'Table in the quiet room.',
      restaurant_id: restaurantIds['Coconut Lagoon'],
      user_id: 1
    }
  });
  const booking4 = await prisma.booking.create({
    ...
    {
      number_of_people: 4,
      booking_time: new Date('2023-05-13T18:00:00Z'),
      booker_email: "peter@gmail.com",
      booker_phone: "1112223333",
      booker_first_name: "Peter",
      booker_last_name: "Lynch",
      booker_occasion: 'Promotion',
      booker_request: 'Table by the window',
      restaurant_id: restaurantIds['Vivaan - fine Indian'],
      user_id: 2
  ...
  await prisma.bookingsOnTables.createMany({
    data: [
      //there is issue here because id and table id is hard coded
      {
  ...
  res.status(200).json({ name: "data has been reseeded" });
}